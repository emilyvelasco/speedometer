...
void loop() {
 ...
 //print the number of seconds since reset:
    ...
    pinState = digitalRead(sensorPin);
      if (pinState == LOW && pinState != oldpinState){
        debounceCount = millis()+50;

      }
        //Serial.print("timer count ="); Serial.println(TimerCount);
        //Serial.print("Pin state ="); Serial.println(pinState);
      
        //Serial.print("Old pin state ="); Serial.println(oldpinState);
      if (debounceCount < millis()){
            pinState = digitalRead(sensorPin);
